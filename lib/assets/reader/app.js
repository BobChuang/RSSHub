...
function setAiSummaryPushSettingsVisible(visible) {
    elements.aiSummaryPushSettings.hidden = !visible;
    elements.aiSummaryWebhookInput.disabled = !visible;
    elements.aiSummaryPushTimeInput.disabled = !visible;
}
...
function resetAiSummaryPushForm(status = '') {
    ...
    setAiSummaryPushSettingsVisible(false);
}

function renderAiSummaryPush(push) {
    const enabled = Boolean(push?.enabled);
    elements.aiSummaryPushEnabledInput.checked = enabled;
    ...
    setAiSummaryPushSettingsVisible(enabled);
    if (push?.lastError) {
...
async function saveAiSummaryPushSettings() {
    ...
    try {
        const push = await saveAiSummaryPush({
            ...
            enabled,
            feedIds,
            nextSendAt: getNextAiSummaryPushAt(sendTime),
            prompt,
            sendTime,
        ...
        setStatus(enabled ? 'AI 定时推送已保存。' : 'AI 定时推送已关闭。');
    } catch (error) {
...
function bindEvents() {
    ...
    elements.aiSummaryPushEnabledInput.addEventListener('change', () => {
        setAiSummaryPushSettingsVisible(elements.aiSummaryPushEnabledInput.checked);
