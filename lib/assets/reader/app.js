...
const aiSummaryPushOffMode = 'off';
...
const elements = {
    ...
    aiSummaryPushTimeField: document.querySelector('#aiSummaryPushTimeField'),
    aiSummaryRealtimePushInput: document.querySelector('#aiSummaryRealtimePushInput'),
    aiSummaryPushSettings: document.querySelector('#aiSummaryPushSettings'),
...
async function getAiSummary(feedId, days, prompt, savePrompt) {
...
async function getAiSummaryForFeeds(feedIds, days, prompt, savePrompt) {
    if (feedIds.length === 1) {
        return getAiSummary(feedIds[0], days, prompt, savePrompt);
    }
...
async function saveAiSummaryPush(payload) {
...
function getDefaultAiSummaryPrompt(feedIds) {
    return feedIds.length > 1 ? defaultMultiAiSummaryPrompt : defaultAiSummaryPrompt;
}

function getAiSummaryPromptForFeeds(feedIds, { fallbackPrompt = '', preferSaved = true } = {}) {
    const savedPrompt = preferSaved ? feedIds.map((feedId) => getFeed(feedId)?.aiSummaryPrompt).find(Boolean) : '';
    return savedPrompt || fallbackPrompt || getDefaultAiSummaryPrompt(feedIds);
}

function saveAiSummaryPromptToLocalFeeds(feedIds, prompt) {
...
function getNextAiSummaryPushAt(sendTime) {
    const [hours, minutes] = String(sendTime || defaultAiSummaryPushTime)
        .split(':')
        .map(Number);
    const next = new Date();
...
function getAiSummaryPushMode() {
    if (elements.aiSummaryRealtimePushInput.checked) {
        return aiSummaryPushRealtimeMode;
    }
    if (elements.aiSummaryPushEnabledInput.checked) {
        return aiSummaryPushSummaryMode;
    }
    return aiSummaryPushOffMode;
}

function setAiSummaryPushSettingsVisible() {
    const mode = getAiSummaryPushMode();
    const visible = mode !== aiSummaryPushOffMode;
    const scheduled = mode === aiSummaryPushSummaryMode;
    elements.aiSummaryPushSettings.hidden = !visible;
    elements.aiSummaryWebhookInput.disabled = !visible;
    elements.aiSummaryPushTimeField.hidden = !scheduled;
    elements.aiSummaryPushTimeInput.disabled = !scheduled;
}

function setAiSummaryPushMode(mode) {
    elements.aiSummaryPushEnabledInput.checked = mode === aiSummaryPushSummaryMode;
    elements.aiSummaryRealtimePushInput.checked = mode === aiSummaryPushRealtimeMode;
    setAiSummaryPushSettingsVisible();
}

function resetAiSummaryPushForm(status = '') {
    setAiSummaryPushMode(aiSummaryPushOffMode);
...
function renderAiSummaryPush(push) {
    const enabled = Boolean(push?.enabled);
    const mode = push?.mode === aiSummaryPushRealtimeMode ? aiSummaryPushRealtimeMode : aiSummaryPushSummaryMode;
    setAiSummaryPushMode(enabled ? mode : aiSummaryPushOffMode);
    ...
    if (push?.lastError) {
        elements.aiSummaryPushStatus.textContent = '上次推送失败：' + push.lastError;
        return;
    }
    if (enabled && mode === aiSummaryPushRealtimeMode) {
        elements.aiSummaryPushStatus.textContent = push?.lastSentAt ? '上次实时推送：' + formatDate(push.lastSentAt) : '有新内容时实时推送。';
        return;
    }
    elements.aiSummaryPushStatus.textContent = enabled && push?.nextSendAt ? '下次发送：' + formatDate(push.nextSendAt) : '';
}
...
async function loadAiSummaryPushForCurrentSelection() {
    ...
    elements.aiSummaryPushStatus.textContent = '正在读取推送设置...';
    try {
        const result = await lookupAiSummaryPush(feedIds);
        if (token !== aiSummaryPushLookupToken) {
            return;
        }
        renderAiSummaryPush(result.push);
    } catch (error) {
        if (token === aiSummaryPushLookupToken) {
            resetAiSummaryPushForm(error instanceof Error ? error.message : '读取推送设置失败。');
        }
    }
}
...
async function saveAiSummaryPushSettings() {
    ...
    const mode = getAiSummaryPushMode();
    const enabled = mode !== aiSummaryPushOffMode;
    ...
    const sendTime = elements.aiSummaryPushTimeInput.value || defaultAiSummaryPushTime;
    const prompt = elements.aiSummaryPromptInput.value.trim() || initialAiSummaryPrompt || defaultAiSummaryPrompt;
    const days = Number(elements.aiSummaryRangeInput.value) === 7 ? 7 : 1;
    elements.saveAiSummaryPushButton.disabled = true;
    elements.aiSummaryPushStatus.textContent = '正在保存推送设置...';
    try {
        const push = await saveAiSummaryPush({
            days,
            enabled,
            feedIds,
            mode: enabled ? mode : aiSummaryPushSummaryMode,
            nextSendAt: mode === aiSummaryPushSummaryMode ? getNextAiSummaryPushAt(sendTime) : undefined,
            prompt,
            sendTime,
        ...
        renderAiSummaryPush(push);
        setStatus(enabled ? (mode === aiSummaryPushRealtimeMode ? '实时推送已保存。' : 'AI 定时推送已保存。') : '推送已关闭。');
    } catch (error) {
        elements.aiSummaryPushStatus.textContent = error instanceof Error ? error.message : '保存推送设置失败。';
    } finally {
...
async function submitAiSummary(event) {
    ...
    const days = Number(elements.aiSummaryRangeInput.value) === 7 ? 7 : 1;
    const prompt = elements.aiSummaryPromptInput.value.trim() || defaultAiSummaryPrompt;
    ...
    try {
        const result = await getAiSummaryForFeeds(feedIds, days, prompt, savePrompt);
        if (savePrompt) {
            initialAiSummaryPrompt = result.promptTemplate || prompt;
            saveAiSummaryPromptToLocalFeeds(feedIds, initialAiSummaryPrompt);
        }
...
function bindEvents() {
    ...
    elements.aiSummaryPushEnabledInput.addEventListener('change', () => {
        if (elements.aiSummaryPushEnabledInput.checked) {
            elements.aiSummaryRealtimePushInput.checked = false;
        }
        setAiSummaryPushSettingsVisible();
    ...
    elements.aiSummaryRealtimePushInput.addEventListener('change', () => {
        if (elements.aiSummaryRealtimePushInput.checked) {
            elements.aiSummaryPushEnabledInput.checked = false;
        }
        setAiSummaryPushSettingsVisible();
        elements.aiSummaryPushStatus.textContent = '';
        if (elements.aiSummaryRealtimePushInput.checked) {
