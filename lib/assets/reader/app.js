...
function setAiSummaryPushEditorVisibility() {
    const scheduled = elements.aiSummaryPushModeInput.value === aiSummaryPushSummaryMode;
    ...
    elements.aiSummaryPushTimezone.textContent = scheduled ? '发送时间按 ' + (editingAiSummaryPushTimezone || '服务器本地时区') + ' 计算。' : '有新内容时立即推送，不使用定时设置。';
}
...
function createAiSummaryPushItem(push) {
    ...
    const details = createText('p', 'ai-summary-push-item-details', (push.timezone || '服务器本地时区') + ' · ' + (push.webhookUrl || '未设置 Webhook'));
    ...
    if (push.lastError) {
        ...
        item.append(createText('p', 'ai-summary-push-item-status', '上次发送：' + formatDate(push.lastSentAt)));
...
function renderAiSummaryPushes(pushes) {
    ...
    if (!pushes.length) {
        elements.aiSummaryPushList.append(createText('p', 'ai-summary-push-empty', '还没有推送任务，可新增每日、每周或实时推送。'));
        return;
    }
    pushes.forEach((push) => {
...
function openAiSummaryPushEditor(push) {
    ...
    elements.aiSummaryPushModeInput.value = push?.mode === aiSummaryPushRealtimeMode ? aiSummaryPushRealtimeMode : aiSummaryPushSummaryMode;
    ...
    elements.aiSummaryPushPromptInput.value = push?.prompt || elements.aiSummaryPromptInput.value.trim() || initialAiSummaryPrompt;
...
async function saveAiSummaryPushSettings() {
    ...
    const mode = elements.aiSummaryPushModeInput.value === aiSummaryPushRealtimeMode ? aiSummaryPushRealtimeMode : aiSummaryPushSummaryMode;
    ...
    const payload = {
        ...
        feedIds,
        mode,
        prompt: elements.aiSummaryPushPromptInput.value.trim() || initialAiSummaryPrompt || getDefaultAiSummaryPrompt(feedIds),
